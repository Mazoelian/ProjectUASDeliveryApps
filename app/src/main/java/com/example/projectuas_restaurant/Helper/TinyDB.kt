...
class TinyDB<T : Any>(appContext: Context?) {
    ...
    fun <T : Any> getListObject(key: String?, clazz: Class<T>): ArrayList<T> {
        val gson = Gson()
        val objStrings = getListString(key)
        val playerList: ArrayList<T> = ArrayList()
        for (jObjString in objStrings) {
            val player: T = gson.fromJson(jObjString, clazz)
